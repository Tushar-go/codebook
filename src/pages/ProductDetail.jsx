...
function ProductDetail() {
   ...
   useEffect(()=>{
    async function fetchProduct(){
      const response = await fetch(`/api/products/${productId}`)  // dont need to add : after / 
        ...
        <section>
          ...
          <div className="flex flex-wrap justify-around">
            <div className="max-w-xl my-3">
              <img className="rounded   h-64" src={image} alt={name} />
            </div>
            <div className="max-w-xl my-3">
              ...
              <p className="my-3">
                ...
                {!inCart && <button onClick={()=> token ? addToCart(product) : navigate("/login") } disabled={ product.in_stock ? "" : "disabled" } className={`inline-flex items-center py-2 px-5 text-lg font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 ${product.in_stock ? "": "cursor-not-allowed"}`}>Add To Cart <i className="ml-1 bi bi-plus-lg"></i></button> }
                {inCart &&   <button onClick={()=> removeToCart(product)} className={`inline-flex items-center py-2 px-5 text-lg font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-800 
